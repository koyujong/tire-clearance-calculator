...
<p>서스펜션 리프트 세팅에 앞서 내가 장착할 타이어 사이즈부터 결정하는 것이 언제나 튜닝의 첫 번째 단계여야 합니다. <a href="/">타이어 튜닝 외경 속도계 오차 계산기</a>를 사용해 보유 차량의 순정 타이어와 목표 타이어 사이의 정확한 제원을 비교해보세요. 직경 차이에 따른 정확한 지상고 상승분(반지름의 법칙)을 계산해 드립니다.</p>`,
  ...
  // 3
  'fitting-35s-on-stock-f150-leveling-kit-ko': `
...
<p>35인치는 직경을 뜻합니다. 12.5인치 폭의 거대한 타이어보다는 **285/75R18(직경 약 34.8인치, 폭 11.2인치)**이나 **295/70R18** 같은 좁은 폭의 35인치 타이어를 선택하세요. 간섭 스트레스를 없앨 수 있습니다.</p>
<h2>결론</h2>
<p>2인치 레벨링과 +18 오프셋 휠, 11~11.5인치 폭의 35인치 타이어 조합이 F-150 순정의 한계치입니다. 작업 전 반드시 <a href="/">계산기</a>로 세팅값을 확인하세요.</p>`,

  // 4
  'best-wheel-offset-12-5-wide-tires-tacoma-ko': `
...
<h2>해결책: 타협과 튜닝</h2>
<p>타코마에 12.5 광폭 타이어를 간섭 없이 넣으려면 **에프터마켓 UCA** 장착과 **Cab Mount Chop (CMC 작업)**이 거의 세트메뉴처럼 요구됩니다. 이 비용이 부담스럽다면 285/70R17(투팔오) 사이즈로 내려오는 것이 현명합니다. 타이어 스펙 변경 시 오차율은 <a href="/">외경 계산기</a>로 확인하세요.</p>`,
...
<p>리기어링은 전/후 액슬(사륜구동의 경우)을 모두 까서 작업해야 하므로 공임 포함 약 200~300만 원 선의 고비용 튜닝입니다. 하지만 37인치를 계획 중이라면 <strong>선택이 아닌 필수</strong> 항목입니다. 무리한 기어 헌팅으로 미션이 박살 나는 것에 비하면 저렴한 투자입니다.</p>`,
  ...
  // 6
  'rotational-mass-how-tire-weight-kills-mpg-ko': `
...
<p>해결책은 무리하게 등급이 높은 두꺼운 타이어를 고집하지 말고, 꼭 필요한 조건(C-load 또는 D-load)의 가벼운 HT나 AT 타이어 모델을 우선 고려하는 것입니다. 외경 계산뿐 아니라 타이어 제조사의 무게 제원을 꼼꼼히 체크하세요.</p>`,
  ...
  // 7
  'ground-clearance-math-3-inch-lift-vs-35s-ko': `
...
<p>타이어는 차축 지상고를 결정하고, 리프트 킷은 타이어가 펜더에 닿지 않도록 공간을 만들어 주며 배가 닿는 것을 방지합니다. 결국 오프로더의 최종 목표는 <strong>가장 큰 타이어를 넣을 수 있는 만큼만 리프트업 하는 것 (LCG 세팅)</strong>입니다. 리프트 킷 구매 전에 먼저 <a href="/">외경 계산기</a>로 목표 타이어 세팅을 점검하세요.</p>`,
  ...
  // 8
  'pizza-cutters-vs-wide-tires-snow-performance-ko': `
...
<p>오버랜더들이 사랑하는 **255/85R16**이나 **235/85R16** 같은 좁고 직경이 큰 타이어를 피자 커터라고 부릅니다.</p>
<ul>
...
<ol>
  ...
  <li><a href="/">타이어 둘레 계산기</a>에서 확인한 새로운 외경 타이어의 밀리미터(mm) 둘레값을 16진수(Hex) 또는 직접 수치로 수정하여 입력합니다. (예: 285/70R17은 2636mm 수준)</li>
...
<h2>타이어 지름르고 후회 안 하는: 구매 전 간섭 측정법</h2>
...
Object.entries(koReplacements).forEach(([slug, newContent]) => {
  // Use non-greedy match to safely replace only the content of the corresponding slug block
  const slugPattern = new RegExp(
    \`(slug:\\s*["'\`]\\s*\${slug}\\s*["'\`][\\s\\S]*?content:\\s*\`)([\\s\\S]*?)(\`\\.trim\\(\\))\`,
    'g'
  );
  ...
  if (content === before) {
    console.error(\`❌ Could not find or replace exactly for slug: \${slug}\`);
  } else {
...
console.log('\\n✅ All 10 Korean blog posts fully updated with exact translations!');
