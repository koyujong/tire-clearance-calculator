...
<h2>선택의 기준</h2>
<p>여러분이 사는 곳에 눈이 자주 오고 포장도로 주행 비율이 압도적으로 높다면, 무리해서 너비 315 급이나 12.50을 장착할 이유가 없습니다. <a href="/">타이어 규격 계산기</a>에 너비(Width)를 변경해 가며 최적의 오차를 가진 좁은 타이어 세팅을 시뮬 시뮬레이션해 보세요.</p>`,
...
Object.entries(koReplacements).forEach(([slug, newContent]) => {
  const startTag = \`slug: "\${slug}"\`;
  const contentFieldStartIdx = content.indexOf('content: \`', content.indexOf(startTag));
  
  if (contentFieldStartIdx === -1) {
    console.error(\`❌ Error finding content field for \${slug}\`);
    return;
  }
  
  const contentStart = content.indexOf('\`', contentFieldStartIdx) + 1;
  const contentEnd = content.indexOf('\`.trim()', contentStart);
  
  if (contentEnd === -1) {
    console.error(\`❌ Error finding end of content for \${slug}\`);
    return;
  }
...
console.log('\\n✅ All 10 Korean blog posts successfully updated using exact string matching!');
